...
class TestRenameItemFields:
    ...
    def test_rename_core_item_fields(self) -> None:
        ...
        env.assert_in_lib_dir(
            "Tag Artist/Tag Album/Tag Artist - Tag Album - 01 Tag Title 1 "
            "(Tag Album Artist) newname.file"
        )

    def test_rename_other_meta_item_fields(self) -> None:
        """Tests that the value of `year`, `month`, `day`, `track`, `tracktotal`,
        `disc`, and `disctotal` populate in renaming.
        """
        env = self.env
    ...
    def test_rename_lyric_comment_item_fields(self) -> None:
        """Tests that the value of `lyrics` and `comments` populate in renaming."""
        env = self.env

        env.config["filetote"]["extensions"] = ".file"
        env.config["paths"]["ext:file"] = "$albumpath/$lyrics ($comments)"

        env.run_cli_command("import")

        env.assert_in_lib_dir("Tag Artist/Tag Album/Tag lyrics (Tag comments).file")
    ...
    def test_rename_track_music_item_fields(self) -> None:
        ...
        env.assert_in_lib_dir(
            "Tag Artist/Tag Album/newname - 8bpm 0_01 (MP3) (80kbps).file"
        )
    ...
    def test_rename_mb_item_fields(self) -> None:
        ...
        env.assert_in_lib_dir(
            "Tag Artist/Tag Album/someID-3 - someID-2 (someID-4) - someID-1 someID-5 - "
            "Tag work musicbrainz id.file"
        )
